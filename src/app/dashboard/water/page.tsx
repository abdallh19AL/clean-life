...
export default function WaterPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data?.water_cups != null) {
        const count = Math.min(data.water_cups, GOAL);
        setCups(Array(GOAL).fill(false).map((_, i) => i < count));
      }
      setLoading(false);
    })();
  }, []);
  ...
  function toggleCup(i: number) {
    const newCups = cups[i]
      ? cups.map((v, idx) => idx < i ? v : false)  // unfill from i outward
  ...
  function reset() {
    ...
    if (userId) persistCups(userId, 0);
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 32 }}>
        ...
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: "#1a1a1a", marginBottom: 2 }}>تتبع شرب الماء</h1>
          <p style={{ fontSize: 13, color: "#aaa" }}>الهدف اليومي: 2.5 لتر (8 أكواب) — يُحفظ تلقائياً</p>
        </div>
      </div>
      ...
      >
        <div style={{ marginBottom: 16 }}>
          <motion.p
            ...
            style={{ fontSize: 64, fontWeight: 900, color: loading ? "#ccc" : "#5B8CBF", lineHeight: 1 }}
          >
            {loading ? "..." : filled}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {loading ? "" : `${motivation.emoji} ${motivation.text}`}
          </motion.p>
        </AnimatePresence>
      </motion.div>
      ...
      >
        ...
        <div className="grid grid-cols-4 gap-4">
          {cups.map((isFilled, i) => (
            <motion.button
              key={i}
              onClick={() => !loading && toggleCup(i)}
              whileHover={{ scale: 1.08, y: -4 }}
              whileTap={{ scale: 0.92 }}
              animate={isFilled ? { scale: [1, 1.12, 1] } : {}}
              style={{
                padding: "18px 0", borderRadius: 18, border: "none",
                backgroundColor: isFilled ? "rgba(91,140,191,0.12)" : "rgba(248,245,240,0.9)",
                outline: isFilled ? "2px solid rgba(91,140,191,0.35)" : "2px solid rgba(190,175,155,0.20)",
        ...
        <p style={{ textAlign: "center", fontSize: 12, color: "#ccc", marginTop: 16 }}>كل كوب = 250 مل (ربع لتر)</p>
      </motion.div>
      ...
      <div className="grid grid-cols-3 gap-[14px] mb-5">
        {[
          ...
          { label: "الهدف",  value: "2.50",                                unit: "لتر", color: "#3D7A5E", bg: "rgba(61,122,94,0.08)"   },
        ].map(({ label, value, unit, color, bg }) => (
          <div key={label} style={{
            backgroundColor: bg, border: `1.5px solid ${color}20`,
